    ...
    <div>
      ...
      >
        ...
        <div slot="action" slot-scope="{text, record}">
          ...
          <router-link :to="`/system/version/1/${record.key}`"  v-auth="`edit`"> <a-icon type="edit" /> 编辑 </router-link>


          <a @click="deleteRecord(record.key)" v-auth="`delete`">
            <a-icon  type="delete" /> 删除 
          </a>
          <router-link :to="`/system/version/0/${record.key}`"  v-auth="`watch`"> <a-icon type="info-circle" /> 详情 </router-link>
        </div>
